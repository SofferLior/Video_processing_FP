...
def map_image_to_costs(image,binary):
    """
    ...
    cost_field[exits] = 0
    cost_field[space] = 1. / (255 * grey_scales[space] + 1e-17)
    return cost_field
...
def _wdt_python(cost_field):
    ...
    while True:
        ...
        for cell in new_candidate_cells:
            ...
            # heapq.heapify(cand_heap)  # marina
...
def _propagate_distance(cell, costs, wdt_field):
    ...
    #if(D<0):
    #    D=0
    if -b - math.sqrt(abs(D)) == 0:
        x_high = (2 * c) / (-b - math.sqrt(abs(D)) + 1e-7)
    else:
        x_high = (2 * c) / (-b - math.sqrt(abs(D)))
    return x_high
